...
class SecretNetworkMCPServer {
  ...
  async start(): Promise<void> {
    ...
    // Check for HTTP mode (experimental)
    const httpMode = process.env.MCP_HTTP_MODE === 'true';
    
    if (httpMode) {
      // Simple HTTP wrapper for remote access (experimental)
      ...
      const httpServer = createServer((req, res) => {
        ...
        if (req.url === '/health') {
          ...
          res.end(JSON.stringify({ 
            ...
            message: 'MCP server running in HTTP mode (experimental)' 
          }));
          return;
        }
        ...
        res.end(JSON.stringify({
          ...
          version: '0.1.0',
          transport: 'http-wrapper',
          note: 'This is an experimental HTTP wrapper. Standard MCP uses stdio transport.'
        }));
      });
      
      httpServer.listen(port, () => {
        this.logger.info('Secret Network MCP Server HTTP wrapper started', {
          mode: 'http-experimental',
          port: port,
          network: this.config.NETWORK,
          note: 'Primary MCP server still uses stdio transport'
        });
      });
    }
    
    // Always start the standard stdio transport (this is the real MCP server)
    ...
    this.logger.info('Secret Network MCP Server started', {
      transport: 'stdio',
      network: this.config.NETWORK,
      httpWrapper: httpMode ? 'enabled' : 'disabled'
    });
  }
